<template>
    <div class="articles-container">
        <div class="article-top">
            <div>{{isAuthor ? '我' : (sex ? '她' : '他')}}的话题</div>
        </div>
        <template v-if="articles.length > 0">
            <div v-for="(article, index) in articles" class="articles-item" :class="{'articles-item-no': index === 0}">
                <h2 class="articles-title"><a :href="`/topic/${article.id}`" target="_blank">{{article.name}}</a>
                    <span class="user-article-status" v-if="isAuthor && article.status != 1" :style="{color: article.status === 2 ? 'red' : 'blue'}">[ {{ article.status | statusName }} ]</span>
                </h2>
                ...
                <div class="wforum-digest" v-html="article.htmlContent"></div>
            </div>
        ...
        asyncData (context) {
            ...
            return request.getUserArticles({
                ...
                for (let i = 0; i < articles.length; i++) {
                    let limit = 100
                    let more = `...&nbsp;&nbsp;<a href="/topic/${articles[i].id}" target="_blank" class="wforum-digest-continue">继续阅读»</a>`
                    articles[i].htmlContent = MD.render(articles[i].content || '')
                ...
                return {
                    ...
                    isAuthor: context.user && context.user.id === parseInt(context.params.id)
                }
            }).catch(err => {
                console.log(err)
                context.error({ statusCode: 404, message: 'Page not found 0x003' })
            })
        },
        mounted () {
