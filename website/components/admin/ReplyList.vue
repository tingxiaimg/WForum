                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: (h, obj) => {
                            return h('div', {
                                domProps: {
                                    innerHTML: MD.render(obj.row.content)
                                }
                            })
                        }
                    },
                    ...
                    },
                    {
                        ...
                        key: 'id',
                        render: (h, obj) => {
                            let sourceName = obj.row.sourceName
                            let path = 'topic'
                            let reply = `/#reply-${obj.row.id}`
                            if (sourceName === 'comment') {
                                reply = `/#reply-${obj.row.parentId}`
                            }
                            return (
                                h('a', {
                                    'class': 'reply-view',
                                    attrs: {
                                        href: `/${path}/${obj.row.articleId}${reply}`,
                                        target: '_blank'
                                    }
                                }, '查看')
                            )
            ...
            onPageChange (pageNo) {
                ...
                Request.getComments({
                    ...
                    if (res.status === ErrorCode.ERROR) {
                        ...
                        self.theTotalCount = res.data.total
                    }
                }).catch(err => {
